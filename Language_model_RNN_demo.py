...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    comment: This layer should use the input_placeholder to index into the
          embedding.
    comment: You might find tf.nn.embedding_lookup useful.
    comment: You might find tf.split, tf.squeeze useful in constructing tensor inputs
    comment: Check the last slide from the TensorFlow lecture.
    comment: Here are the dimensions of the variables you will need to create:
  ...
  def add_projection(self, rnn_outputs):
    ...
    comment: Here are the dimensions of the variables you will need to
          create 
  ...
  def add_training_op(self, loss):
    ...
    comment: Use tf.train.AdamOptimizer for this model.
  ...
  def add_model(self, inputs):
    ...
    comment: Use a zeros tensor of shape (batch_size, hidden_size) as
          initial state for the RNN. Add this to self as instance variable

          self.initial_state
  
          (Don't change variable name)
    comment: Add the last RNN output to self as instance variable

          self.final_state

          (Don't change variable name)
    comment: Make sure to apply dropout to the inputs and the outputs.
    comment: Use a variable scope (e.g. "RNN") to define RNN variables.
    comment: Perform an explicit for-loop over inputs. You can use
    ...
    comment: Here are the dimensions of the various variables you will need to
          create:
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  comment: Create a feed-dictionary and use sess.run() to execute the model. Note
        that you will need to use model.initial_state as a key to feed_dict
  comment: Fetch model.final_state and model.predictions[-1]. (You set
        model.final_state in add_model() and model.predictions is set in
        __init__)
  comment: Store the outputs of running the model in local variables state and
